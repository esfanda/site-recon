...
echo [2/3] Reading Gemini + PageSpeed keys from config\secrets.yaml...
...
for /f "usebackq tokens=1,* delims=:" %%A in (`findstr /r "pagespeed_api_key:" config\secrets.yaml`) do set PAGESPEED_LINE=%%B
set PAGESPEED_LINE=%PAGESPEED_LINE: =%
if not "%PAGESPEED_LINE%"=="" (
    set SITE_RECON_DEMO_PAGESPEED_KEY=%PAGESPEED_LINE%
    echo     PageSpeed key found — will enable PageSpeed on the demo.
) else (
    echo     WARN: pagespeed_api_key missing — PageSpeed will show N/A on the demo.
)
